 ...
 * @private
 * @param    {Number} [code] http status code
 * @param    {Object | Buffer | String | Number | Error} [body] the content to send
 * @param    {Object} [headers] any add'l headers to set
...
Response.prototype.__send = function __send() {
    ...
    if (format === false) {
        assert.ok(typeof body === 'string' || typeof body === 'number' 
            || Buffer.isBuffer(body),
                  'res.sendRaw() accepts only strings, numbers, or buffers');
        return _flush(body);
    }
